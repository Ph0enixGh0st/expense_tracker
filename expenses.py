...
def get_today_statistics():
    ...
    if not result[0]:
        return "No expense records for today."
    ...
    return (f"Today expenses:\n"
            f"Today total = {all_today_expenses} EUR\n"
            f"Spent = {base_today_expenses} EUR from budget limit of {_get_budget_limit()} EUR\n\n"
...
def get_month_statistics():
    ...
    return (f"Current month expenses:\n"
            f"Total = {all_today_expenses} EUR\n"
            f"Spent — {base_today_expenses} EUR from budget limit of "
            f"{now.day * _get_budget_limit()} EUR")
...
def _parse_message(raw_message):
    ...
    if not regexp_result or not regexp_result.group(0) \
        ...
        raise exceptions.NotCorrectMessage(
            "Bot couldn't parse the message. Please use the following format: "
            "\n20 taxi")
